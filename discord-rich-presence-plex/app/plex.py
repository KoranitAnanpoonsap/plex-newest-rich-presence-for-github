    ...
    def handleAlert(self, alert: Alert) -> None:
        ...
        if mediaType == "movie":
            ...
            if item.title != item.grandparentTitle:
                stateStrings.append(item.title)
            thumb = item.grandparentThumb
        # MODIFICATION START: Replaced the 'track' media type block
        elif mediaType == "track":
            title = shortTitle = item.title
            
            # Set hover text for album art to be song title + audio info
            largeText = title
            if item.media and item.media[0].parts and item.media[0].parts[0].streams:
                audio_stream = next((stream for stream in item.media[0].parts[0].streams if stream.type == 2), None)
                if audio_stream:
                    audio_codec = audio_stream.codec
                    audio_bitdepth = audio_stream.bitDepth
                    audio_samplerate = round(audio_stream.samplingRate / 1000) if hasattr(audio_stream, "samplingRate") and audio_stream.samplingRate else None
                    
                    info_parts = []
                    if audio_codec: info_parts.append(str(audio_codec).upper())
                    if audio_bitdepth: info_parts.append(f"{audio_bitdepth}-bit")
                    if audio_samplerate: info_parts.append(f"{audio_samplerate}kHz")
                    
                    if info_parts:
                        largeText = f"{title} ({' / '.join(info_parts)})"

            # Build the state string (artist, album, year) based on config
            state_parts = []
            if config.config["display"]["artist"] and (item.originalTitle or item.grandparentTitle):
                state_parts.append(item.originalTitle or item.grandparentTitle)
            
            if config.config["display"]["album"]:
                album_part = item.parentTitle
                if config.config["display"]["year"]:
                    parent = self.server.fetchItem(item.parentRatingKey)
                    if parent.year:
                        album_part += f" ({parent.year})"
                state_parts.append(album_part)
            
            if state_parts:
                stateStrings.append(" - ".join(state_parts))
        ...
        if config.config["display"]["buttons"]:
            ...
            for button in config.config["display"]["buttons"]:
                ...
                # MODIFICATION START: Added handler for dynamic YouTube search button
                if buttonType == "youtube_search":
                    search_query = ""
                    if mediaType == "track":
                        search_query = urllib.parse.quote_plus(f"{item.grandparentTitle} {item.title}")
                    elif mediaType in ["movie", "episode"]:
                        search_query = urllib.parse.quote_plus(f"{shortTitle} trailer")
                    if search_query:
